"""Synthetic-data generators for the simulated extensions of Olist.

Two callers:
- `scripts/simulate.py` — the dev/full-scale simulator anchored on the real
  ~99k Olist orders.
- `scripts/seed_minimal.py` — the CI-friendly seed that builds a small
  self-contained dataset (no Olist needed).

Both produce the same deliberately-injected anomaly: the `rel_mobile_v2`
regression that drops mobile conversion ~40% during 2018-04-15..05-10.
"""
...
def gen_releases() -> pd.DataFrame:
    rows = [
        ...
        ("rel_002", "search", "v2.1", "2017-06-01", None, "New search ranking algorithm."),
        ("rel_003", "desktop_checkout", "v3.0", "2017-09-12", None, "Desktop checkout redesign."),
        ...
        ("rel_006", "search", "v2.2", "2018-07-15", None, "Search relevance improvements."),
    ]
    df = pd.DataFrame(
        rows,
...
def gen_campaigns(rng: np.random.Generator, n: int = 50) -> pd.DataFrame:
    ...
    df["name"] = (
        df["channel"].str.replace("_", " ").str.title() + " " + df["start_ts"].dt.strftime("%Y-%m")
    )
...
def gen_web_events(rng: np.random.Generator, scale: float = 1.0) -> pd.DataFrame:
    ...
    if len(orders) == 0:
        raise SystemExit("No orders in raw.orders. Seed orders first.")
    ...
    if scale < 1.0:
        orders = orders.sample(frac=scale, random_state=int(rng.integers(2**31))).reset_index(
            drop=True
        )
    ...
    event_types_c = np.array(
        ["page_view", "page_view", "add_to_cart", "begin_checkout", "purchase"]
    )
    ...
    nc_evcount = rng.integers(1, 6, n_nc)
    ...
    bug_offsets = rng.integers(0, int((bug_end_naive - bug_start_naive).total_seconds()), n_bug)
...
def gen_support_tickets(rng: np.random.Generator, n: int = 5000) -> pd.DataFrame:
    ...
    texts = [rng.choice(TICKET_TEMPLATES[c]).format(int(rng.integers(3, 21))) for c in cats]

    open_offsets = rng.integers(1, 30 * 86400, n)
    opened = sample["purchase_ts"].to_numpy() + pd.to_timedelta(open_offsets, unit="s").to_numpy()
